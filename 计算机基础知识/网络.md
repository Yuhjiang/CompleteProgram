...
### TCP
- 提供了面向连接、可靠的字节流服务
- 在一个TCP连接中，仅有双方彼此通信，广播和多播不适用TCP
- TCP使用校验和，确认和重传机制保证传输的可靠性。_(校验和：伪首部（源IP地址，目的IP地址，传输协议，
TCP报文长度），TCP数据，TCP头)_
- TCP给数据分节进行排序，并使用累积确认和保证数据的顺序和不重复
- TCP使用滑动窗口机制来实现流量控制，通过动态改变窗口的大小进行拥塞控制，[滑动窗口原理](https://juejin.im/post/5c9f1dd651882567b4339bce)
- Socket服务端
    1. 创建socket。socket()
    2. 设置socket属性，setsocketopt()
    3. 绑定IP地址，端口到socket，bind()
    4. 开始监听，listen()
    5. 接收客户端的请求，accept()
    6. 收发数据，send()&recv()
    7. 关闭网络连接close()
...
### UDP
- 缺乏可靠性，不提供确认，序列号，超时重传等机制，UDP 数据报可能在网络中被复制，被重新排序。
即 UDP 不保证数据报会到达其最终目的地，也不保证各个数据报的先后顺序，也不保证每个数据报只到达一次
- 数据报长度有限制，如果一个数据报正确到达目的地，该数据报的长度将随数据一起传递给接收方
- UDP是无连接的，没有握手创建连接的过程
- UPD支持多播和广播（广播和局域网内所有的人说话，多播将网络中同一业务的主机进行逻辑上分组数据
收发仅在同一个分组)
- Socket服务端
    1. 创建socket，socket()
    2. 设置socket属性，setsocketopt()
...
- HEAD: 向服务器所要与GET请求相一致的响应，但是response主体不会返回
- GET: 向特定的资源发出请求
- POST: 提交指定资源的数据并处理请求
- PUT: 更新指定资源内容
- DELETE: 请求服务器删除指定资源
- TRACE: 用于激发一个远程的，应用层的请求消息回路，允许客户端直到请求链的另一端接收什么并利用哪些数据去测试或诊断
