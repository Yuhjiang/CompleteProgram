...
- `pfmerge`，如果想要把多个pf计数数值累加到一起形成一个新的pf，可以使用pfmerge。比如把两个
内容差不多的页面的访问量合并
...
节约大量的存储空间。
- 原理：数据结构式一个大型的位数组和几个一样的无偏hash函数（无偏的意思是能够把元素的hash值
算得比较均匀，让元素被hash映射到位数组中的位置比较随机）。
    - 向布隆过滤器添加key时，会使用多个hash函数进行计算，算得一个索引值，然后对位数组长度
    进行取模运算得到一个位置，每个hash函数都会得到一个不同的位置，再把位数组这几个位置置1，
    就这完成了add操作
    - 查询一个key存在不存在时，使用hahs计算位置，如果位置有一个位0，说明key不存在，如果都为
    1，就可以认为这个key极可能存在（因为其他key算出来的位置可能把这些位置置1了）。
